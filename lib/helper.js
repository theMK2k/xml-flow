...
const tryGuttingNode = (node, keepArrays) => {
  ...
  // MK2k: don't gut $attrs, because this is inconsistent
  if (
    keys.length === 1 &&
    firstKey !== "$name" &&
    firstKey !== "id" &&
...
const simplifyNode = (node, dropName, keepArrays) => {
  ...
  // MK2k: we don't want to merge attributes into the stripped object, because this is inconsistent
  // const merged = tryMergingAttrs($attrs, stripped);
  const merged = Object.assign(stripped, { $attrs });
  ...
  if (node.$markup) {
    node.$markup.forEach((item) => {
      if (typeof item === "string") {
        ...
        output[key] = moosh(
          output[key],
          simplifyAll(item, true, keepArrays),
          keepArrays
        );
      }
    });
