...
const flow = function xmlFlow(inStream, options = defaults) {
  ...
  saxStream.on("opentag", (node) => {
    ...
    // console.log(">> PUSHING to Stack:", node.name, topNode);
    stack.push(topNode);
  });
  ...
  saxStream.on("closetag", (tagName) => {
    ...
    if (emitter.listeners(`tag:${tagName}`).length) {
      if (pauseOnEmit) {
        inStream.unpipe(saxStream); // Pause the stream - the event handler needs to resume it
      }

      emitter.emit(
        `tag:${tagName}`,
        simplifyNodes
          ? helper.simplifyNode(topNode, false, useArrays > SOMETIMES)
          : topNode,
        saxStream._parser.position // Add the current byte position
      );
    }

    //Pop stack, and add to parent node
    // console.log("<< POPPING from Stack:", topNode);
    stack.pop();
    if (stack.length > 0) {
  ...
  emitter.pause = function pause() {
    // inStream.pause();
    inStream.unpipe(saxStream);
    inStream.pause();
  };

  emitter.resume = function resume() {
    // inStream.resume();
    if (!inStream._readableState.pipes.length) {
...
flow.toXml = function toXml(
  obj,
  { indent = "", selfClosing = true, escape = helper.escape, nodeName } = {}
) {
